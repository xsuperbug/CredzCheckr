...
from color_config import INFO, found, not_found, LESS
...
class finger_print:
    ...
    def other_check(self, url):
        ...
        if req_fav.status_code == 200:
            ...
            print("   {} Favicon.ico hash: {}".format(INFO, hash_fav))
            for fg in favinger:
                if hash_fav == fg:
                    techno_found = True
                    print("  {} {} found".format(found, favinger[fg]))
                    return favinger[fg]
        if not techno_found:
            print("   {} favicon not found in template".format(not_found))
                    ...
                    for di in default_input.read().splitlines():
                        try:
                            if di.split(":")[0] == p["name"] and username_input != di.split(":")[0]:
                                print("   {} input user found: {}".format(found, p["name"]))
                                username_input = p["name"]
                            if di.split(":")[1] == p["name"] and password_input != di.split(":")[1]:
                                print("   {} input password found: {}".format(found, p["name"]))
            ...
            if username_input and password_input:
                ...
                print("   {} Inputs not found".format(LESS))
                return "n"
